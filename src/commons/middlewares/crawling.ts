  ...
  async getNearbyHospitals() {
    ...
    // }).then(response => {
      ...
      $('#area_dashboards').each((idx, element) => {
        const $data = cheerio.load(element);
        let name = $data('#area_dashboards > div > div.dash_header > div > span > a').text().replace(/\s+/g,' ');
        let ungeup = $data('#area_dashboards > div > div.dash_data > div:nth-child(2) > table > tbody > tr > td:nth-child(1) > div.data_data.emer_bed.data_td_O001 > span:nth-child(2)').text().replace(/\s+/g,' ');
        let susul = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(2) > td:nth-child(2) > div.data_data.data_td_O022`).text().replace(/\s+/g,' ');
        let singyeong = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(1) > td:nth-child(4) > div.data_data.data_td_O011`).text().replace(/\s+/g,' ');
        let sinseng = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(1) > td:nth-child(2) > div.data_data.data_td_O008`).text().replace(/\s+/g,' ');
        let hyungbu = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(1) > td:nth-child(7) > div.data_data.data_td_O016`).text().replace(/\s+/g,' ');
        let ilban = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(1) > td:nth-child(1) > div.data_data.data_td_O017`).text().replace(/\s+/g,' ');
        let ibwon = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(2) > td:nth-child(1) > div.data_data.data_td_O038`).text().replace(/\s+/g,' ');
        let naeggwa = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(1) > td:nth-child(3) > div.data_data.data_td_O006`).text().replace(/\s+/g,' ');
        let weggwa = $data(`#rltmList_${idx} > table > tbody > tr:nth-child(1) > td:nth-child(5) > div.data_data.data_td_O007`).text().replace(/\s+/g,' ');
