...
export class ReportsService {
  ...
  async createDummyReport() {
    ...
    let symptom_list = ['소실된 의식', '심부전', '뇌경색 증상', '사지 마비', '의식 변화',
                        ...
                        '체중감소'];
    let latitude = 35; let longitude = 127; let count = 0;
    for(let i = 40; i <= 79; i++) { // 병원
      for (let j = 1; j <= 5; j++) { // 환자
        for(let k = 1; k <= 5; k++) { // 증상도
          for(let l = 0; l < 100; l++) {
            let hospital_id = i; let patient_id = j; let symptom_level = k;
            let symptom = [];
            let symptom_count = Math.floor(Math.random() * (5 - 1 + 1)) + 1;
            while(symptom.length < symptom_count) {
              let num = Math.floor(Math.random() * (40 - 0 + 1)) + 0;
              if (symptom.every((e) => symptom_list[num] !== e)){
                symptom.push(symptom_list[num]);
              }
            }
            latitude += 0.00010000; longitude += 0.00010000;
            if(latitude > 38) {
              latitude = 35;
            }
            if(longitude > 129) {
