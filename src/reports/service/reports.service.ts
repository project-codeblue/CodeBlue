...
export class ReportsService {
  ...
  async createDummyReport() {
    ...
    const symptom_list: string[] = ['소실된 의식','심부전','뇌경색 증상','사지 마비','의식 변화',
                                    ...
                                    '코피','고열','음식 섭취 곤란','알레르기 반응','가려운 발진','체중감소',];
    const blood_type_list: BloodType[] = [BloodType.A, BloodType.B, BloodType.AB, BloodType.O];
    const range: AgeRange[] = [AgeRange.노년, AgeRange.성인, AgeRange.영유아, AgeRange.임산부, AgeRange.청소년];                     
    // let latitude = 36;
    // let longitude = 127;
    let count = 0;
    // for (let i = 1; i <= 10; i++) { // 병원
      for (let j = 0; j < 4; j++) { // 혈액형
        for (let k = 0; k < 5; k++) { // 연령대
          for (let l = 1; l <= 5; l++) { // 증상도
            for (let m = 0; m < 10; m++) {
              // const hospital_id: number = i;
              const blood_pressure: string = '130/80';
