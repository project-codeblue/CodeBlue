...
export class ReportsService {
  ...
  async createDummyReport() {
    ...
    const symptom_list: string[] = ['소실된 의식', '심부전', '뇌경색 증상', '사지 마비', '의식 변화',
                          ...
                          '체중감소'];
    let latitude: number = 35; let longitude: number = 127; const count: number = 0;
    for (let i = 40; i <= 79; i++) { // 병원
      for (let j = 1; j <= 5; j++) { // 환자
        for (let k = 1; k <= 5; k++) {// 증상도
          for (let l = 0; l < 100; l++) {
            const hospital_id: number = i; const patient_id: number = j; const symptom_level: number = k;
            const symptom: string[] = [];
            const symptom_count: number = Math.floor(Math.random() * (5 - 1 + 1)) + 1;
